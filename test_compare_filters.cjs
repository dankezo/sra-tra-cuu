...
(async()=>{
 ...
 await page.evaluate(()=>{
  ...
  window.SraExcel={build:async sheets=>{window.exported=sheets;return new Uint8Array([1]);},downloadBlob:async()=>'download'};
 ...
 await page.locator('#compare-rules-pop summary').click();
 ...
 await page.locator('#compare-rules-pop').evaluate(el=>{el.open=true;});
